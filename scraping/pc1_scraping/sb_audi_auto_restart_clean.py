...
class AutoRestartScraper:
    def __init__(self):
        self.all_results = []
        # SOAX proxy - Replace with your credentials
        self.proxy = "YOUR_SOAX_USERNAME:YOUR_SOAX_PASSWORD@proxy.soax.com:9000"
    ...
    def scrape_batch(self, urls_batch, batch_num):
        ...
        with SB(uc=True, test=True, proxy=self.proxy, headless=False,
            ...
            print(f"[BATCH {batch_num}] TURK TELEKOM Mobile proxy connected!")
            ...
            # DİĞER 5 URL'İ SIRAYLA AÇ (1'er saniye arayla)
            ...
            for tab in tab_info:
                ...
                try:
                    # JavaScript'ten topladığı veriyi al - Cloudflare kontrolü ile
    ...
    def scrape_single_url(self, sb, url, batch_num):
        ...
        try:
            ...
            bot_detected = False  # Always false now
...
def load_urls(count=5, random_select=True, mercedes=False):
    ...
    if mercedes:
        # Mercedes CLA data - Audi range: 92-184  
    ...
    try:
        ...
        if mercedes and start_idx is not None and end_idx is not None:
            ...
            print(f"[MERCEDES] Using Audi range: {start_idx}-{end_idx-1} ({len(valid_urls)} URLs)")
...
def mercedes_continuous_scraping():
    """Mercedes CLA Continuous Scraping - Audi range: 92-183"""
    print("[MERCEDES] CLA CONTINUOUS SCRAPING - AUDI")
    print("="*60)
    
    # YENI AUDI URLs - 13k URL dosyası
    MERCEDES_FILE = r"C:\Users\emirh\Downloads\sahibinden_urls_ALL_2025-08-20T19-31-32-806Z.txt"
    AUDI_START = 1265  # 165. URL'den devam (1100 + 165 = 1265)
    AUDI_END = 4565   # 1265'ten başlayıp 3300 URL çek (1265 + 3300 = 4565)
    BATCH_SIZE = 5
    ...
    try:
        ...
        audi_urls = all_urls[AUDI_START:AUDI_END]
        print(f"[RANGE] Audi URLs: {AUDI_START}-{AUDI_END-1} ({len(audi_urls)} total)")
    ...
    print(f"[START] Processing {len(audi_urls)} URLs in batches of {BATCH_SIZE}")
    print("="*60)
    
    while current_index < len(audi_urls):
        # Get next batch
        end_index = min(current_index + BATCH_SIZE, len(audi_urls))
        batch_urls = audi_urls[current_index:end_index]
        
        print(f"\n[BATCH {batch_number}]")
        print(f"   URLs: {current_index+AUDI_START}-{end_index+AUDI_START-1} ({len(batch_urls)} URLs)")
        print(f"   Progress: {end_index}/{len(audi_urls)} URLs ({end_index/len(audi_urls)*100:.1f}%)")
        ...
        print(f"   [TOTAL] Processed: {total_processed}/{len(audi_urls)} URLs")
        ...
        if current_index < len(audi_urls):
            print(f"   [NEXT] Moving to next batch...")
            time.sleep(2)
    
    print("\n" + "="*60)
    print("[COMPLETE] AUDI MERCEDES SCRAPING FINISHED!")
...
def main():
    ...
    print("Strategy: SADECE 1 BATCH TEST (6 URLs paralel)")
