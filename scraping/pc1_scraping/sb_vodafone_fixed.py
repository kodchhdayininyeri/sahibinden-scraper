...
class AutoRestartScraper:
    def __init__(self):
        self.all_results = []
        # Vodafone Turkey SOAX proxy - Replace with your credentials
    ...
    def scrape_batch(self, urls_batch, batch_num):
        ...
        with SB(uc=True, test=True, proxy=self.proxy, headless=False,
            ...
            print(f"[BATCH {batch_num}] VODAFONE TURKEY Mobile proxy connected!")
    ...
    def save_results(self, filename=None):
        ...
        print(f"Batches: {len(batch_stats)} ({self.batch_size} URLs each)")
        print(f"{'='*80}")
...
def load_urls(count=5, random_select=True, mercedes=False):
    ...
    if mercedes:
        # Mercedes CLA data - Vodafone range: 0-92
    ...
    try:
        ...
        if mercedes and start_idx is not None and end_idx is not None:
            ...
            print(f"[MERCEDES] Using Vodafone range: {start_idx}-{end_idx-1} ({len(valid_urls)} URLs)")
...
def mercedes_continuous_scraping():
    """Mercedes CLA Continuous Scraping - Vodafone range: 0-91"""
    print("[MERCEDES] CLA CONTINUOUS SCRAPING - VODAFONE")
    print("="*60)
    
    # YENI AUDI URLs - 13k URL dosyası
    MERCEDES_FILE = r"C:\Users\emirh\Downloads\sahibinden_urls_ALL_2025-08-20T19-31-32-806Z.txt"
    VODAFONE_START = 4565  # Audi'den sonra devam
    VODAFONE_END = 7865   # 4565'ten başlayıp 3300 URL çek (4565 + 3300 = 7865)
    BATCH_SIZE = 5
    ...
    try:
        ...
        vodafone_urls = all_urls[VODAFONE_START:VODAFONE_END]
        print(f"[RANGE] Vodafone URLs: {VODAFONE_START}-{VODAFONE_END-1} ({len(vodafone_urls)} total)")
    ...
    print(f"[START] Processing {len(vodafone_urls)} URLs in batches of {BATCH_SIZE}")
    print("="*60)
    
    while current_index < len(vodafone_urls):
        # Get next batch
        end_index = min(current_index + BATCH_SIZE, len(vodafone_urls))
        batch_urls = vodafone_urls[current_index:end_index]
        
        print(f"\n[BATCH {batch_number}]")
        print(f"   URLs: {current_index}-{end_index-1} ({len(batch_urls)} URLs)")
        print(f"   Progress: {end_index}/{len(vodafone_urls)} URLs ({end_index/len(vodafone_urls)*100:.1f}%)")
        ...
        print(f"   [TOTAL] Processed: {total_processed}/{len(vodafone_urls)} URLs")
        ...
        if current_index < len(vodafone_urls):
            print(f"   [NEXT] Moving to next batch...")
            time.sleep(2)
    
    print("\n" + "="*60)
    print("[COMPLETE] VODAFONE MERCEDES SCRAPING FINISHED!")
...
def main():
    ...
    print("Strategy: SADECE 1 BATCH TEST (1 URL)")
