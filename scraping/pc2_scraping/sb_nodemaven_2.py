"""
NodeMaven Auto-Restart Scraper #2 - Her 5 URL'de yeni IP
NodeMaven proxy ile otomatik restart (İkinci instance)
"""
...
class AutoRestartScraper:
    def __init__(self):
        self.all_results = []
        # NodeMaven ROTATING proxy formatı - SeleniumBase format (İkinci instance)
    ...
    def handle_bot_detection(self, sb, url, batch_num):
        ...
        try:
            ...
            print(f"[FIRE ONE-SHOT #{self.bot_detect_count}] Bot detect! TEK ATIŞ temizleme...")
            ...
            try:
                ...
                try:
                    sb.execute_script("""
                        ...
                        console.log('FIRE Bot detection mesajı gönderildi');
                    """)
                ...
                if len(cookies_mid) >= len(cookies_before):
                    ...
                    sb.execute_script("""
                        ...
                        console.log('FIRE JavaScript cookie temizleme yapıldı');
                    """)
    ...
    def inject_data_collector(self, sb):
        ...
        return sb.execute_script("""
            ...
            return 'NodeMaven #2 data collector injected';
        """)
    ...
    def scrape_batch(self, urls_batch, batch_num):
        ...
        with SB(uc=True, test=True, proxy=self.proxy, headless=False,
            ...
            print(f"[BATCH {batch_num}] NODEMAVEN #2 Mobile proxy connected!")
            ...
            if "just a moment" in title or "dakika" in title:
                print("[FIRE CLOUDFLARE] Challenge detected, waiting...")
                sb.sleep(15)
            ...
            if first_url_status.get('status') == 'cloudflare':
                print("[FIRE CLOUDFLARE] İlk URL'de Cloudflare tespit edildi, 15s bekleniyor...")
                sb.sleep(15)
                print("[FIRE CLOUDFLARE] Çözüldü, devam ediliyor...")
                ...
                try:
                    ...
                    # Bot detection tespit edildi mi?
                        ...
                        sb.execute_script("""
                            ...
                            localStorage.clear();
                            sessionStorage.clear();
                            console.log('BOT JavaScript cookie temizleme yapıldı');
                        """)
                    ...
                    if collected_data and not collected_data.get('cloudflare_detected'):
                        ...
                        collected_data['proxy_type'] = 'nodemaven_2_mobile_parallel'
    ...
    def process_all_urls(self, all_urls, max_urls=None):
        ...
        print(f"\n{'='*80}")
        print(f"NODEMAVEN #2 AUTO-RESTART SCRAPER")
    ...
    def save_results(self, filename=None):
        ...
        if not filename:
            ...
            filename = f"nodemaven_2_results_{timestamp}.json"
        ...
        final_data = {
            'summary': {
                ...
                'proxy_type': 'NodeMaven #2 Turkish Mobile'
            },
        ...
        print(f"Batches: {len(batch_stats)} (1 URLs each)")
        print(f"{'='*80}")
        
        return filename

def load_urls(count=1, random_select=True):
...
def mercedes_continuous_scraping():
    """Mercedes CLA Continuous Scraping - NodeMaven_2 range: 368-461"""
    print("[MERCEDES] CLA CONTINUOUS SCRAPING - NODEMAVEN_2")
    print("="*60)
    
    # Mercedes range for NodeMaven_2
    MERCEDES_FILE = r"C:\Users\YB\Desktop\algofact\mercedes_cla_data.txt"
    NODEMAVEN2_START = 368
    NODEMAVEN2_END = 462
    BATCH_SIZE = 5
    ...
    try:
        ...
        nodemaven2_urls = all_urls[NODEMAVEN2_START:NODEMAVEN2_END]
        print(f"[RANGE] NodeMaven_2 URLs: {NODEMAVEN2_START}-{NODEMAVEN2_END-1} ({len(nodemaven2_urls)} total)")
    ...
    print(f"[START] Processing {len(nodemaven2_urls)} URLs in batches of {BATCH_SIZE}")
    print("="*60)
    
    while current_index < len(nodemaven2_urls):
        # Get next batch
        end_index = min(current_index + BATCH_SIZE, len(nodemaven2_urls))
        batch_urls = nodemaven2_urls[current_index:end_index]
        
        print(f"\n[BATCH {batch_number}]")
        print(f"   URLs: {current_index+NODEMAVEN2_START}-{end_index+NODEMAVEN2_START-1} ({len(batch_urls)} URLs)")
        print(f"   Progress: {end_index}/{len(nodemaven2_urls)} URLs ({end_index/len(nodemaven2_urls)*100:.1f}%)")
        ...
        print(f"   [TOTAL] Processed: {total_processed}/{len(nodemaven2_urls)} URLs")
        ...
        if current_index < len(nodemaven2_urls):
            print(f"   [NEXT] Moving to next batch...")
            time.sleep(2)
    
    print("\n" + "="*60)
    print("[COMPLETE] NODEMAVEN_2 MERCEDES SCRAPING FINISHED!")
...
def main():
    """Ana fonksiyon"""
    print("NODEMAVEN #2 AUTO-RESTART MULTI-TAB SCRAPER STARTING...")
    ...
    print("Strategy: 5 URL BATCH TEST - PARALLEL TABS")
