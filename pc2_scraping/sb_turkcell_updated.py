"""
SOAX Auto-Restart Scraper - Her 4 URL'de yeni IP
Otomatik restart ile sürekli IP rotation
Bot detection sistemi iyileştirildi
"""
...
class AutoRestartScraper:
    def __init__(self):
        self.all_results = []
        # Turkcell SOAX proxy
        self.proxy = "YOUR_SOAX_USERNAME:mobile;tr;turkcell@proxy.soax.com:9000"
    ...
    def scrape_batch(self, urls_batch, batch_num):
        """3 URL'lik batch'i gerçek paralel scrape et - JavaScript ile arka planda"""
        print(f"\n{'='*70}")
        print(f"BATCH {batch_num} - TRUE PARALLEL (JAVASCRIPT)")
        print(f"URLs: {len(urls_batch)} | Total Processed: {(batch_num-1)*3}")
        print(f"{'='*70}")
        ...
        extension_path = r"C:\Users\YB\Desktop\algofact\cookietemizleme"
        
        with SB(uc=True, test=True, proxy=self.proxy, headless=False,
               extension_dir=extension_path) as sb:
            print(f"[BATCH {batch_num}] TURKCELL Mobile proxy connected!")
            ...
            # BOT DETECTION - TEMPORARILY DISABLED FOR SPEED
            ...
            for i in range(1, len(urls_batch)):
                ...
                delay = random.uniform(1, 2)  # 1-2 saniye random gecikme
                time.sleep(delay)
                print(f"[ TAB {i}] Açıldı, {delay:.1f}s beklendi...")
            ...
            for tab in tab_info:
                ...
                try:
                    ...
                    if collected_data and not collected_data.get('cloudflare_detected') and not collected_data.get('bot_detected'):
                        # Batch ve success bilgisi ekle
                        collected_data['batch'] = batch_num
                        collected_data['tab_index'] = tab['index']
                        collected_data['proxy_type'] = 'soax_mobile_parallel'
                        ...
                        if collected_data.get('title') and (collected_data.get('price') or collected_data.get('specs')):
                            ...
                            successful_tabs.append(collected_data)
                        else:
                            collected_data['success'] = False
                            print(f"   ERROR Tab {tab['index']}: Veri eksik")
                            failed_tabs.append(tab)
                    else:
                        print(f"   [WARNING] Tab {tab['index']}: Veri toplanamadı (detection aktif)")
                        failed_tabs.append(tab)
            ...
            # RETRY LOGIC: Failed tablar varsa tekrar dene
            if failed_tabs:
                print(f"\n[RETRY] {len(failed_tabs)} tab failed, retrying in 5s...")
                time.sleep(5)
                ...
                for tab in failed_tabs:
                    try:
                        ...
                        print(f"   [RETRY] Tab {tab['index']}: Re-injecting collector...")
                        
                        # Re-inject data collector
                        self.inject_data_collector(sb)
                        time.sleep(2)
                        ...
                        if collected_data and collected_data.get('title'):
                            collected_data['batch'] = batch_num
                            collected_data['tab_index'] = tab['index']
                            collected_data['proxy_type'] = 'soax_mobile_parallel'
                            collected_data['success'] = True
                            print(f"   SUCCESS RETRY Tab {tab['index']}: {collected_data.get('title', '')[:50]}...")
                            retry_successful.append(collected_data)
                        else:
                            print(f"   FAILED RETRY Tab {tab['index']}: Still no data")
                            final_failed.append(tab)
                            
                    except Exception as e:
                        print(f"   ERROR RETRY Tab {tab['index']}: {str(e)[:50]}")
                        final_failed.append(tab)
                ...
                # Add final failed tabs to results
                for tab in final_failed:
                ...
                print(f"[RETRY COMPLETE] {len(retry_successful)} recovered, {len(final_failed)} still failed")
            
            # Add all successful results from first round
            for result in successful_tabs:
                batch_results.append(result)
        
        print(f"\n[BATCH {batch_num} COMPLETE] Processed {len(batch_results)} URLs")
        return batch_results
    
    def scrape_single_url(self, sb, url, batch_num):
        """Tek URL'i scrape et"""
        try:
            ...
            result = {
                ...
                'batch': batch_num,
                'proxy_type': 'turkish_mobile_auto_restart'
            }
            
            # BOT DETECTION CHECK - İLK ÖNCE!
            current_title = sb.get_title().lower()
            page_text = ""
            try:
                page_text = sb.get_text("body")[:500].lower()
            except:
                pass
                
            # Debug: title ve page content'i göster
            print(f"[DEBUG] Title: {current_title[:80]}")
            print(f"[DEBUG] Page text: {page_text[:100]}")
            
            # Sahibinden bot detection patterns
            bot_detected = False
            login_redirect = False
            
            if 'just a moment' in current_title or 'dakika' in current_title:
                bot_detected = True
                print(f"[ BOT DETECT] Cloudflare detected in title")
            elif 'user login' in current_title:
                login_redirect = True
                bot_detected = True
                print(f"[🔐 LOGIN REDIRECT] Forced login page detected")
            elif 'olağan dışı erişim' in page_text or 'olagan disi erisim' in page_text:
                bot_detected = True
                print(f"[ BOT DETECT] Sahibinden unusual access detected")
            elif 'erişim engellendi' in page_text or 'erisim engellendi' in page_text:
                bot_detected = True
                print(f"[ BOT DETECT] Sahibinden access blocked")  
            elif 'robot olmadığınızı' in page_text or 'checking your browser' in page_text:
                bot_detected = True
                print(f"[ BOT DETECT] Bot verification detected")
            elif 'in order to provide you a better experience' in page_text:
                login_redirect = True
                bot_detected = True
                print(f"[🔐 LOGIN REDIRECT] Better experience login detected")
            elif 'cf_clearance' in page_text or 'captcha' in page_text:
                bot_detected = True
                print(f"[ BOT DETECT] Captcha/clearance detected")
            elif len(page_text) < 100:  # Çok az içerik varsa bot detect olabilir
                bot_detected = True
                print(f"[ BOT DETECT] Very little page content (possible bot detection)")
            
            # Eğer bot detect varsa, hemen temizle ve retry yap
            if bot_detected:
                print(f"[ BOT DETECT] Bot detection algılandı! Temizleme başlatılıyor...")
                
                # Login redirect ise özel strateji
                if login_redirect:
                    print(f"[🔐 LOGIN BYPASS] Attempting direct URL access...")
                    try:
                        # Direkt URL'e git (redirect'i bypass et)
                        original_url = url
                        sb.open(original_url)
                        sb.sleep(5)
                        
                        # Hala login sayfasındaysa cookie temizle
                        new_title = sb.get_title().lower()
                        if 'login' in new_title:
                            print(f"[🔐 STILL LOGIN] Cookie cleanup required")
                            cleanup_success = self.handle_bot_detection(sb, url, batch_num)
                        else:
                            print(f"[🔐 LOGIN BYPASSED] Direct access successful")
                ...
                if not cleanup_success:
                    result['error'] = "Bot detected, cleanup failed"
                    return result
                # Cleanup basariliysa devam et ve veri cikar
            
            # Title check
            title = sb.get_title()
            if any(word in title.lower() for word in ["dakika", "loading", "just a moment"]):
                print("[CLOUDFLARE] Challenge detected, waiting...")
                sb.sleep(15)
                title = sb.get_title()
            ...
            try:
                ...
                for selector in price_selectors:
                    if sb.is_element_present(selector):
                        price_text = sb.get_text(selector)
                        if price_text and "TL" in price_text:
                            # Sadece TL içeren ilk satırı al
                            lines = price_text.split('\n')
                            for line in lines:
                                clean_line = line.strip()
                                if "TL" in clean_line and len(clean_line) < 50:
                                    # Sadece rakam ve TL kısmını al
                                    import re
                                    price_match = re.search(r'[\d.,]+\s*TL', clean_line)
                                    if price_match:
                                        result['price'] = price_match.group()
                                    else:
                                        result['price'] = clean_line
                                    break
                            if 'price' in result:
                                break
            except:
            ...
            # Extract location
            try:
                if sb.is_element_present(".classifiedInfo h2"):
                    location_element = sb.find_element(".classifiedInfo h2")
                    location_links = location_element.find_elements("tag name", "a")
                    if location_links:
                        locations = [link.text.strip() for link in location_links if link.text.strip()]
                        if locations:
            ...
            try:
                            ...
                            pageTrackData.customVars.forEach(item => {
                                if (item.name && item.value) {
                                    const carSpecs = [
                                        'Marka', 'Seri', 'Model', 'Yıl', 'Yakıt Tipi', 
                                        'Vites', 'KM', 'Motor Gücü', 'Renk', 'Kimden',
                                        'Kasa Tipi', 'Motor Hacmi'
                                    ];
                                    if (carSpecs.includes(item.name)) {
                                        specs[item.name] = item.value.toString();
                                    }
                                }
                            });
                ...
                if specs:
                    result['specs'] = specs
                    print(f"[DATA] Extracted {len(specs)} specs")
            except:
                pass
            ...
            # Success check
            data_points = ['title', 'price', 'specs', 'location']
            found_data = sum(1 for key in data_points if key in result and result[key])
            
            if found_data >= 2:
                result['success'] = True
                print(f"[SUCCESS] {result.get('title', 'Data extracted')[:40]}... ({found_data}/4)")
            else:
                result['error'] = f"Insufficient data extracted ({found_data}/4)"
                print(f"[FAILED] Only {found_data}/4 data points")
    ...
    def save_results(self, filename=None):
        ...
        if not filename:
            ...
            filename = f"auto_restart_results_{timestamp}.json"
        ...
        print(f"Batches: {len(batch_stats)} (4 URLs each)")
        print(f"{'='*80}")
        
        return filename

def load_urls(count=5, random_select=True):
    """URL'leri yükle - yeni dosyadan"""
    url_file = r"C:\Users\YB\Desktop\algofact\sahibinden_urls_ALL_2025-08-13T06-41-14-675Z.txt"
...
def mercedes_continuous_scraping():
    """Mercedes CLA Continuous Scraping - Turkcell range: 276-367"""
    print("[MERCEDES] CLA CONTINUOUS SCRAPING - TURKCELL")
    print("="*60)
    
    # YENI AUDI URLs - 13k URL dosyası (PC2 için)
    MERCEDES_FILE = r"C:\Users\YB\Desktop\algofact\sahibinden_urls_ALL_2025-08-20T19-31-32-806Z.txt"
    TURKCELL_START = 10100# PC2 560'dan başlasın
    TURKCELL_END = 11200# 560-709 arası 150 URL çek
    BATCH_SIZE = 5
    ...
    try:
        ...
        turkcell_urls = all_urls[TURKCELL_START:TURKCELL_END]
        print(f"[RANGE] Turkcell URLs: {TURKCELL_START}-{TURKCELL_END-1} ({len(turkcell_urls)} total)")
    ...
    print(f"[START] Processing {len(turkcell_urls)} URLs in batches of {BATCH_SIZE}")
    print("="*60)
    
    while current_index < len(turkcell_urls):
        # Get next batch
        end_index = min(current_index + BATCH_SIZE, len(turkcell_urls))
        batch_urls = turkcell_urls[current_index:end_index]
        
        print(f"\n[BATCH {batch_number}]")
        print(f"   URLs: {current_index+TURKCELL_START}-{end_index+TURKCELL_START-1} ({len(batch_urls)} URLs)")
        print(f"   Progress: {end_index}/{len(turkcell_urls)} URLs ({end_index/len(turkcell_urls)*100:.1f}%)")
        ...
        try:
            from db_manager_pc2_fixed import SahibindenDB
            db = SahibindenDB()
        ...
        print(f"   [TOTAL] Processed: {total_processed}/{len(turkcell_urls)} URLs")
        ...
        if current_index < len(turkcell_urls):
            print(f"   [NEXT] Moving to next batch...")
            time.sleep(2)
    
    print("\n" + "="*60)
    print("[COMPLETE] TURKCELL MERCEDES SCRAPING FINISHED!")
...
def main():
    """Ana fonksiyon"""
    print("AUTO-RESTART MULTI-TAB SCRAPER STARTING...")
